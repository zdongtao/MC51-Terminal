...
//#define  _Nop()  _nop_(),_nop_(),_nop_(),_nop_(),_nop_(),_nop_(),_nop_(),_nop_(),_nop_(),_nop_(),_nop_()      /*定义空指令*/
#define  READ       0x01
#define  WRITE      0x00
#define  IIC_ADDR   0x5A        //丛机地址
...
********************************************************************/
#if 1
bit ISendStrExt(uchar xdata *s,uchar num)
{
     uchar i;

     Start_I2c();               //启动总线
     SendByte(IIC_ADDR & 0xFE);      //发送器件地址,WRITE
     if(ack==0)return(0);
...
注意：    使用前必须已结束总线。
********************************************************************/
bit IRcvStrExt(uchar xdata *s,uchar num)
{
   uchar i;

   Start_I2c();
   SendByte(IIC_ADDR | READ);
   if(ack==0)return(0);
